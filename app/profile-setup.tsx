      ...
      case 2:
        return (
          <View className="flex-1">
            <Typography variant="h1" weight="bold" className="text-[32px] leading-10 mb-4 text-center text-[#121915]">How many weeks{"\n"}pregnant are you?</Typography>
            <Typography variant="body" className="text-center text-gray-500 mb-10">If you aren't sure, give your best guess.</Typography>
            
            <View className="items-center my-10">
              <Typography variant="h1" weight="bold" className="text-[100px] leading-[110px] text-[#121915]">{weeks}</Typography>
              <Typography variant="h3" className="text-2xl text-gray-500">Weeks</Typography>
            </View>
            ...
            <View className="flex-row items-center gap-5 mb-2">
              ...
              <View className="flex-1 h-10 justify-center relative">
                ...
                <View className="h-2 bg-primary rounded-full absolute" style={{ width: `${(weeks / 42) * 100}%` }} />
                <View className="w-6 h-6 rounded-full bg-primary border-4 border-white absolute shadow-sm" style={{ left: `${(weeks / 42) * 100}%`, marginLeft: -12 }} />
              </View>
            ...
            <View className="flex-row justify-between px-2 mt-2">
              <Typography variant="caption" className="text-gray-400">0 Weeks</Typography>
              <Typography variant="caption" className="text-gray-400">42 Weeks</Typography>
            </View>
          </View>
        );
      case 3:
        return (
          <View className="flex-1">
            <Typography variant="h1" weight="bold" className="text-[32px] leading-10 mb-4 text-center text-[#121915]">Have you ever had any of these conditions?</Typography>
            ...
            <TouchableOpacity className="items-center mt-2">
               <Typography variant="h3" weight="bold" className="text-setup-blue">None of these apply to me</Typography>
            </TouchableOpacity>
          </View>
        );
      case 4:
        return (
          <View className="flex-1">
            <Typography variant="h1" weight="bold" className="text-[32px] leading-10 mb-4 text-center text-[#121915]">Is this your first pregnancy?</Typography>
            
            <View className="flex-row justify-between mt-10 mb-10 gap-4">
              <TouchableOpacity 
                className={`flex-1 bg-white rounded-[40px] p-[30px] items-center border-2 shadow-sm relative ${isFirstPregnancy === true ? 'border-primary' : 'border-[#F1F5F9]'}`}
              ...
              >
                ...
                <Typography variant="h2" weight="bold" className="text-[22px]">Yes</Typography>
              ...
              <TouchableOpacity 
                className={`flex-1 bg-white rounded-[40px] p-[30px] items-center border-2 shadow-sm relative ${isFirstPregnancy === false ? 'border-primary' : 'border-[#F1F5F9]'}`}
              ...
              >
                ...
                <Typography variant="h2" weight="bold" className="text-[22px]">No</Typography>
            ...
            <Typography variant="body" className="text-center text-gray-500 px-5 leading-5">
    ...
    <SafeAreaView className="flex-1 bg-[#F9FAFB]">
      <Screen backgroundColor="#F9FAFB">
        <View className="flex-row items-center justify-between px-6 pt-4 mb-2">
          ...
          <Typography variant="h2" weight="bold" className="text-[22px]">Profile Setup</Typography>
          <View className="w-7" />
        </View>

        <View className="px-10 mb-8">
          <View className="flex-row justify-between mb-2">
            <Typography variant="caption" className="text-gray-500 uppercase tracking-widest text-[10px]">
              {step === 4 ? 'Final Step' : `STEP ${step} OF ${totalSteps}`}
            </Typography>
            <Typography variant="caption" weight="bold" className="text-[#1A212E]">
              {step} of {totalSteps}
            </Typography>
          </View>
          <View className="h-[6px] bg-[#E2E8F0] rounded-full overflow-hidden">
            <View className="h-full bg-primary rounded-full shadow-sm" style={{ width: `${(step / totalSteps) * 100}%` }} />
          </View>
        </View>
        ...
        <View className={`px-10 pb-10 ${SCREEN_HEIGHT < 700 ? 'pb-4' : 'pb-8'}`}>
          <TouchableOpacity 
            className={`bg-primary h-[70px] rounded-[35px] flex-row items-center justify-center gap-3 shadow-md ${isCreatingProfile || (step === 4 && isFirstPregnancy === null) ? 'opacity-50' : ''}`}
            activeOpacity={0.8}
            onPress={handleNext}
...
function ConditionItem({ title, subtitle, selected, onPress }: any) {
  return (
    <TouchableOpacity 
      className={`bg-white rounded-[35px] py-5 px-6 flex-row items-center justify-between border shadow-sm ${selected ? 'border-primary bg-[#F7FFF9]' : 'border-[#F1F5F9]'}`}
      onPress={onPress}
      activeOpacity={0.7}
    >
      <View className="flex-1">
        <Typography variant="h3" weight="bold" className="text-lg mb-0.5">{title}</Typography>
        <Typography variant="caption" className="text-gray-500 text-sm">{subtitle}</Typography>
      </View>
      <View className={`w-7 h-7 border rounded-full justify-center items-center ${selected ? 'border-primary bg-white' : 'border-gray-300'}`}>
